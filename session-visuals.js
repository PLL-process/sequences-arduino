...
(() => {
  ...
  const imageSrc = `images/seance-${String(n).padStart(2, "0")}-reelle.png?v=1`;

  const style = document.createElement("style");
  style.textContent = ".realSessionVisual{width:100%;height:100%;border-radius:12px;overflow:hidden;background:#fff}.realSessionVisual img{display:block;width:100%;height:100%;object-fit:cover;transform:scale(var(--image-zoom,1));background:#fff}.realSessionNote{margin:.75rem 0 0;padding:.75rem .85rem;border:1px solid #31536a;border-left:5px solid var(--session-accent,#22d3ee);border-radius:11px;background:#07131f;color:#dce9ef;line-height:1.45}";
  ...
  visual.style.setProperty("--image-zoom", n === 1 ? "1" : "1.45");
  visual.setAttribute("role", "img");
  visual.setAttribute("aria-label", "Infographie réaliste de la séance " + n + " : " + titles[n]);
  visual.innerHTML = `<img src="${imageSrc}" width="1280" height="720" alt="">`;
