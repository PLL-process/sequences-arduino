...
(() => {
  ...
  const VERSION = "2026-07-broches-aide-execution-v3";
  const models = {
    1: {
      guided: `# 1 ACQUÉRIR : interroger le capteur d’humidité branché sur A0
# Écris l’instruction complète sur la ligne suivante.


# 2 MÉMORISER : stocker la mesure dans une variable nommée humidite
# Combine l’affectation et la lecture sur une seule ligne.


# 3 COMMUNIQUER : afficher le contenu de la variable humidite
...
# Écris l’instruction sur la ligne suivante.
`,
      standard: `# Lire le capteur d’humidité branché sur A0 et mémoriser la mesure dans humidite.

# Afficher la valeur mémorisée.

# Maintenir la pompe arrêtée pendant l’observation.
`,
      expert: `# Mission : observer la mesure provenant de A0 et garantir l’arrêt de la pompe.
`,
      reference: `humidite = lire_humidite(A0)
afficher(humidite)
stop()`
    },
    2: {
...
afficher(humidite, reservoir, lumiere)`
  ...
  if (!editor.value.trim() || shouldRefresh || (/_{3,}/.test(editor.value) && !current.attemptedV2)) setStarter(mode());
  ...
  const helpBySession = {
    1: `<p class="help-intro"><strong>But :</strong> transférer la méthode vers A0 sans recopier une solution toute faite.</p>
      <div class="help-steps">
        <article style="--help:#60a5fa"><span>1</span><h4>ACQUÉRIR</h4><p>Une fonction de lecture interroge un capteur branché sur une entrée analogique. La broche est écrite entre parenthèses.</p></article>
        <article style="--help:#c084fc"><span>2</span><h4>MÉMORISER</h4><p>Le signe <code>=</code> range le résultat dans une variable placée à gauche.</p></article>
        <article style="--help:#67e8f9"><span>3</span><h4>COMMUNIQUER</h4><p><code>afficher(...)</code> reçoit le nom de la variable dont on veut montrer la valeur.</p></article>
        <article style="--help:#fb7185"><span>4</span><h4>SÉCURISER</h4><p><code>stop()</code> impose l’état sûr : D6 reste à LOW et la pompe est arrêtée.</p></article>
      </div>
      <h4>Exemple de transfert sur une autre entrée — ne pas recopier tel quel</h4>
      <pre>luminosite = lire_lumiere(A1)\nafficher(luminosite)\nstop()</pre>
      <p><strong>À toi d’adapter :</strong> l’éditeur demande le capteur d’humidité branché sur <strong>A0</strong>, et non le capteur de lumière branché sur A1.</p>
      <details><summary>Fonctions reconnues</summary><ul><li><code>lire_humidite(broche)</code></li><li><code>lire_lumiere(broche)</code></li><li><code>lire_reservoir(broche)</code></li><li><code>afficher(...)</code></li><li><code>arroser(duree)</code></li><li><code>stop()</code></li><li><code>alerter(message)</code></li></ul></details>`,
  ...
  const checkProgram = code => {
    ...
    if (id === 1) { readHumidity(); if (humidity && !new RegExp(`\\bafficher\\s*\\(\\s*${humidity[1]}\\s*\\)`).test(source)) errors.push(`La variable ${humidity[1]} doit être affichée avec le même nom.`); requireStop(); }
  ...
  if (actionRow && !document.getElementById("runCodeV3")) {
    ...
    run.id = "runCodeV3"; run.type = "button"; run.className = "btn primary"; run.textContent = "▶ Exécuter";
