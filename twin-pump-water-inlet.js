...
(() => {
  ...
  if (!svg || svg.querySelector("#twinPumpWaterInletV2")) return;

  // Ajouter un marqueur invisible afin d’éviter une double exécution.
  const NS = "http://www.w3.org/2000/svg";
  const marker = document.createElementNS(NS, "g");
  marker.id = "twinPumpWaterInletV2";
  marker.setAttribute("display", "none");
  svg.appendChild(marker);

  // Déplacer encore légèrement la pompe vers la droite.
  const pump = svg.querySelector("#pumpV5");
  if (pump) pump.setAttribute("transform", "translate(40 45)");

  // Dessiner l’arrivée d’eau en L inversé avec une marge visible au bord droit.
  const oldInletRoute = "M922 258 V325 H970 V470 H800";
  const newInletRoute = "M922 258 H985 V470 H977";

  // Recaler la sortie de pompe vers la plante après le nouveau déplacement.
  const oldOutletRoute = "M800 480 H735 V510 H180 V420";
  const newOutletRoute = "M820 480 H735 V510 H180 V420";
  ...
  if (pumpPlus) pumpPlus.setAttribute("cx", "900");
  if (pumpZero) pumpZero.setAttribute("cx", "900");

  // Recaler les liaisons électriques afin qu’elles restent connectées à la pompe.
  [...svg.querySelectorAll("path")].forEach(path => {
    const route = path.getAttribute("d") || "";
    if (route === "M670 305 H720 V375 H880 V435") {
      path.setAttribute("d", "M670 305 H720 V375 H900 V435");
    }
    if (route === "M880 507 V544 H635 V483") {
      path.setAttribute("d", "M900 507 V544 H635 V483");
    }
  });
})();
